...
class ChatbotService:
    ...
    def _apply_schedule_to_abilities(self, username: str, mentoring_end_bonus: float = 1.0):
        ...
        print(f"[STAMINA_DEBUG] 스케줄에서 운동 시간 추출: {exercise_hours}시간 (전체 스케줄: {schedule})")
        if exercise_hours > 0:
            # 현재 체력을 직접 가져와서 운동 시간만큼 더하기 (정확히 +exercise_hours)
            current_stamina = self._get_stamina(username)
            print(f"[STAMINA_DEBUG] 운동 전 체력: {current_stamina}, 운동 시간: {exercise_hours}")
            new_stamina = min(100, current_stamina + exercise_hours)  # 정확히 운동 시간만큼 증가
            actual_increase = new_stamina - current_stamina
            self._set_stamina(username, new_stamina)
            print(f"[STAMINA] {username}의 체력이 {current_stamina}에서 {new_stamina}로 증가했습니다. (운동 {exercise_hours}시간, 실제 증가: +{actual_increase})")
            if actual_increase < exercise_hours:
                print(f"[STAMINA_WARN] 체력이 최대값(100)에 도달하여 {exercise_hours - actual_increase}시간만큼 증가하지 못했습니다.")
            stamina = new_stamina  # 이후 능력치 계산에 업데이트된 체력 사용
        else:
            print(f"[STAMINA_DEBUG] 운동 시간이 0이므로 체력 증가 없음")
    ...
    def _advance_one_week(self, username: str, mentoring_end: bool = False) -> dict:
        ...
        return {
            ...
            "exam_scores": exam_scores if exam_month else None
        }
    ...
    def _check_exam_in_period(self, start_date: str, end_date: str) -> str:
        """
        주어진 기간(시작일부터 종료일까지) 동안 시험 월에 진입했는지 확인
        반환값: 시험 월 (예: "2024-03") 또는 None

        시험 월 진입 조건:
        - 6월 모의고사: start_date가 5월이고 end_date가 6월이거나, 기간 중에 6월로 진입
        - 9월 모의고사: start_date가 8월이고 end_date가 9월이거나, 기간 중에 9월로 진입
        - 11월 수능: start_date가 10월이고 end_date가 11월이거나, 기간 중에 11월로 진입
        """
        from datetime import datetime
        try:
            ...
            # 기간 중에 시험 월로 진입했는지 확인
            for exam_month in exam_months:
                # start_date의 월이 exam_month보다 작고, end_date의 월이 exam_month이거나 크면 진입한 것
                # 또는 start_date가 exam_month이면 (이미 해당 월에 있음) 진입한 것으로 간주
                if start.month < exam_month <= end.month:
                    month_str = f"{exam_month:02d}"
                    print(f"[EXAM] {exam_month}월 진입 감지: {start_date} ~ {end_date} -> {year}-{month_str}")
                    return f"{year}-{month_str}"

            print(f"[EXAM] 시험 월 진입 없음: {start_date} ~ {end_date}")
            return None
        except Exception as e:
...
숫자 하나만 답변해주세요 (예: 2, -1, 0 등)."""
    ...
    def _build_system_prompt(self, username: str = "사용자") -> str:
        ...
        try:
            ...
            if new_state == "daily_routine":
                ...
                if user_message.strip().lower() != 'init':
                    ...
                    if conv_count >= 5:
                        # _advance_one_week 메서드를 사용하여 능력치 증가 및 주차 진행 처리
                        # 멘토링 종료가 아니므로 mentoring_end=False
                        week_result = self._advance_one_week(username, mentoring_end=False)
                        current_week = self._get_current_week(username)
                        
                        # 시험 결과는 week_result에서 가져옴
                        exam_month = week_result.get('exam_month') if week_result else None
                        exam_scores = week_result.get('exam_scores') if week_result else None
                        exam_result = week_result.get('exam') if week_result else None
                        exam_scores_text = exam_result.get('text', '') if exam_result else ""
                        
                        week_passed = True
                        
                        # 1주 기간 동안 시험이 있었는지 확인 (week_result에서 가져온 정보 사용)
                        # exam_scores는 _advance_one_week에서 이미 계산되었으므로 재계산하지 않음
                        if exam_month:
            ...
            if state_info and state_changed:
                ...
                if subjects_selected:
                    ...
                    # 리스트인 경우 문자열로 변환
                    if isinstance(handler_fixed_reply, list):
                        reply = "\n\n".join(handler_fixed_reply)
                        print(f"[HANDLER_FIXED_REPLY] handler의 fixed_reply (리스트) 사용: {handler_fixed_reply} -> '{reply}'")
                    else:
                    ...
                    if not to_states or len(to_states) == 0:
                        ...
                        print(f"[FIXED_REPLY] {new_state} state 진입 - fixed_reply 사용: '{reply[:100]}...'")

                    print(f"[FIXED_REPLY] LLM 호출 건너뛰기")
            elif state_info and not state_changed:
            ...
            try:
                if new_state == "dialogue":
                    ...
                    similarity, metadata = None, None  # 대화 로그 RAG는 similarity 없음
                    has_context = (context is not None and context.strip())
                    if has_context:
                        print(f"[DIALOGUE_RAG] 대화 로그 기반 RAG 사용 성공")
                    else:
                        print(f"[DIALOGUE_RAG] 대화 로그가 없거나 비어있음")
                else:
                    ...
                    # similarity가 float인지 확인하여 has_context 설정
                    has_context = (context is not None and similarity is not None)
                    if has_context:
                        print(f"[CHROMADB_RAG] RAG 검색 성공")
                    else:
                        print(f"[CHROMADB_RAG] RAG 검색 결과 없음")
            except Exception as e:
...
---\n\n"""
            ...
            if week_passed:
                # 호감도에 따른 공부하러 가는 메시지 생성 (나중에 LLM 응답 뒤에 추가)
            ...
            # [4] LLM 응답 생성 (week_passed일 때도 먼저 사용자 메시지에 대한 응답 생성)
            if True:  # week_passed 여부와 관계없이 LLM 호출
                # [4] LLM 응답 생성
                print(f"\n{'='*50}")
                ...
                # RAG 검색 결과 출력
                if has_context:
                    print(f"[RAG] Context found: True")
                    # similarity가 None이 아닐 때만 float 포맷으로 출력
                    if similarity is not None:
                        print(f"[RAG] Similarity: {similarity:.4f}")
                    else:
                        print(f"[RAG] Similarity: N/A (대화 로그 기반 RAG)")
                    print(f"[RAG] Context: {str(context)[:100]}...")
                else:
                    print(f"[RAG] Context found: False")
                ...
                if exam_strategy_processed and exam_strategy_user_input:
                    if not self.client:
                        ...
                        try:
                            # 전략에 대한 자연스러운 응답 생성
                            system_prompt = "당신은 학생입니다. 쌤(멘토)이 시험 전략을 알려주면 그 전략을 이해하고 확신하거나 확인하는 것처럼 자연스럽게 응답하세요."
                ...
                if mock_exam_advice_user_input is not None and mock_exam_advice_reply is None:
                    if not self.client:
                        ...
                        try:
                            # 조언에 대한 자연스러운 응답 생성
                            if mock_exam_advice_is_good:
                                system_prompt = "당신은 학생입니다. 쌤(멘토)의 좋은 조언을 듣고 감사하고 기뻐하는 반응을 자연스럽게 표현하세요."
                                advice_prompt = f"쌤이 '{mock_exam_advice_user_input}'라고 조언을 해주셨고, 이 조언이 도움이 되었습니다. 감사하고 기뻐하는 반응을 해주세요. (30자 이내로 간단히)"
                            else:
                                system_prompt = "당신은 학생입니다. 쌤(멘토)의 부적절한 조언을 듣고 당황하거나 어색해하는 반응을 자연스럽게 표현하세요."
                ...
                if not self.client and not processed:
                    ...
                    try:
                        # 시스템 프롬프트 생성 (캐릭터 설정, 역할 지침, 대화 예시 포함, 평균 백분위 기반 말투 조정)
            ...
            # 주차 완료 시 LLM 응답 뒤에 공부하러 가는 메시지 추가
            if week_passed and auto_study_message and reply:
                # LLM 응답 뒤에 공부하러 가는 메시지 추가
                reply = f"{reply}\n\n{auto_study_message}"
                print(f"[WEEK_PASSED] LLM 응답 뒤에 공부하러 가는 메시지 추가: {auto_study_message}")
