...
def generate_weakness_message(subject: str, score_data: Dict) -> str:
    ...
    weakness_examples = {
        "국어": [
            ...
            "국어 문법 개념은 외웠는데 문제로 나오면 헷갈렸어요.",
            "비문학 지문에서 보기 내용을 제대로 비교하지 못해서 오답이 많았어요.",
            "문학 작품의 상징이나 표현법을 파악하지 못했어요.",
            "화법 영역에서 대화 흐름을 놓쳐서 질문 의도를 잘못 이해했어요.",
            "비문학 과학·경제 지문처럼 배경지식이 필요한 글이 나오면 당황했어요."
        ],
        "수학": [
            ...
            "문제를 꼼꼼히 안 읽어서 단위를 잘못 적용했어요.",
            "문항당 시간 조절이 안 돼서 마지막 3문제를 찍었어요.",
            "수학 개념은 이해했는데, 응용 문제에서 막혔어요.",
            "문제의 조건을 빠뜨려서 틀렸어요. 집중력이 떨어졌던 것 같아요.",
            "모의고사에서는 긴장해서 평소보다 실수가 많았어요."
        ],
        "영어": [
            ...
            "영어 듣기에서 집중이 흐트러져서 두세 문제를 놓쳤어요.",
            "독해 지문이 길어질수록 앞 내용이 기억이 안 났어요.",
            "접속사나 관계대명사 문제에서 문법 구조를 잘못 파악했어요.",
            "지문 속 함축된 의미(비유, 반어 등)를 잘 못 잡았어요.",
            "시간이 부족해서 마지막 3문제는 감으로 풀었어요."
        ],
        "탐구1": [
            ...
            "탐구1에서 개념 간의 관계를 혼동했어요. 정의는 알지만 연결이 안 됐어요.",
            "문제에서 단서를 잘못 읽어서 다른 개념으로 착각했어요.",
            "기출 문제와 다르게 꼬인 응용 문제를 보면 멘붕이 와요.",
            "암기 위주로 공부해서 원리를 묻는 문제를 놓쳤어요.",
            "시간이 부족해서 마지막 계산 문제는 못 풀었어요."
        ],
        "탐구2": [
            ...
            "탐구2에서 그래프 읽는 법을 잘 몰라서 틀렸어요.",
            "이론은 알지만 응용 상황에 적용이 안 됐어요.",
            "복합 개념이 섞인 문항에서 하나라도 헷갈리면 전부 틀리더라고요.",
            "실험 조건이 바뀌면 어떻게 되는지 예측을 못 했어요.",
            "탐구2에서 계산 과정은 맞는데 선택지를 잘못 고랐어요."
        ]
    }
...
def generate_september_subject_problem(subject: str, score_data: Dict) -> str:
    """
    9월 모의고사 과목별 취약점 메시지 생성
    generate_weakness_message를 사용하여 취약점을 제시합니다. (6월과 동일한 로직)
    """
    return generate_weakness_message(subject, score_data)
