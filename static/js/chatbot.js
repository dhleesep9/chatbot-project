...
async function sendMessage(isInitial = false) {
  ...
  try {
    ...
    // 엔딩 이미지 설정 (다른 업데이트 함수들보다 먼저 실행하여 이미지가 덮어쓰이지 않도록)
    ...
    if (data.image && sideImage) {
      ...
      if (isEndingState || isExamFeedbackState) {
        // 애니메이션 건너뛰기 (이미지 고정)
        console.log(`[IMAGE] 엔딩/피드백 상태 - 이미지 고정: ${data.image}`);
      } else {
...
function saveGameState(data) {
  try {
    ...
    if (data.image !== undefined) gameState.image = data.image; // 엔딩 이미지 저장
  ...
  try {
    ...
    if (savedGameState) {
      ...
      if (gameState.image) {
        const sideImage = document.querySelector(".side-image");
        if (sideImage) {
          sideImage.src = gameState.image;
          console.log(`[LOAD] 엔딩 이미지 복원: ${gameState.image}`);
        }
      }
